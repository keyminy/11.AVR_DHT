...
void I2C_LCD_TEST(char* scm);

#define SLA_W (0x27 << 1) // I2C LCD주소는 0x27인데 << 1 로 하는 이유는
// write모드를 유지하기 위해한다(write는 0이죠, read는 high죠)

void I2C_LCD_TEST(char* scm){
	while(1){
		I2C_write_byte(SLA_W,'8'); // 27번지에 '8'를 보냄 = 0x38 = 0011_1000
		I2C_write_byte(SLA_W,'7'); // 27번지에 '7'를 보냄 = 0x37 = 0011_0111
		// 100ms를 하는 이유는 오실로스코프 파형을 빨리 보기 위해 > 
		I2C_LCD_write_string_XY(0,0,scm); //0번째 line에 0번째 컬럼에 값을쓴다
		// '\n'을 쓰면 깨진다
		//I2C_LCD_write_string_XY(1,0,"MK_YEO");
		_delay_ms(100);
	}
}
...
{
	uint8_t i;
	for(i=0; string[i]; i++) //"hello!!\0n에서 \0을 만날때까지 반복"
